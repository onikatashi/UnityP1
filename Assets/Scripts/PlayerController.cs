...
public class PlayerController : MonoBehaviour
{
    ...
    public float speed = 5f;       // 유닛 이동속도

    private Vector3 targetPosition;     // 목표 위치

    private Camera mainCamera;          // 메인 카메라

    public float paddingX = 0.03f;       // UV 좌표는 0 ~ 1사이, 작은값으로 수치 조정해야 한다.
    ...
    void Move1()
    {
        // 1. 입력 받기 (키보드, 마우스 등 입력은 Input매니저가 담당)
    ...
    void Move2()
    {
        ...
        //Space.World: 월드좌표계 기준 이동 (게임 세게의 절대 좌표) -> 디폴트값
    ...
    void Move3()
    {
        //MoveTowerds는 현재 위치에서 목표 위치로 일정 속도로 이동
        //MoveTowerds(현재위치, 목표위치, 속도 * 시간)
    ...
    void MoveInScreen()
    {
        ...
        // 스크린 좌표: 모니터 해상도의 픽셀
        // 뷰포트 좌표: 카메라 사각뿔 끝에 있는 사각형 - 왼쪽하단(0, 0), 우측상단 (1, 1)
